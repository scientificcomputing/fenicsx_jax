...
from dolfinx_external_operator import (
    FEMExternalOperator, functionspace

)
import pytest
import ufl
from fenicsx_jax.fem import create_real_functionspace
from fenicsx_jax.fem import compile_external_operator_form, pack_external_operator_data
...
def du_NN_dtheta_impl(gdim, x, theta):
    ...
    if gdim == 1:
        out = np.array([x_vec[0]*np.cos(theta_vec[0]*x_vec[0]), np.zeros_like(x_vec[0]),
                        np.zeros_like(x_vec[0]), np.zeros_like(x_vec[0]) ]).T
    else:
        out = np.array([x_vec[0]*np.cos(theta_vec[0]*x_vec[0])*np.sin(theta_vec[1]*x_vec[1]),
                        x_vec[1]*np.sin(theta_vec[0]*x_vec[0])*np.cos(theta_vec[1]*x_vec[1]),
                        np.zeros_like(x_vec[0]),
                            np.zeros_like(x_vec[1]) ]).T
    return out.reshape(-1).copy()


def u_NN_external(gdim, derivatives):
    if derivatives == (0,0):
...
@pytest.mark.parametrize("cell_type", [dolfinx.mesh.CellType.interval, dolfinx.mesh.CellType.triangle, dolfinx.mesh.CellType.quadrilateral, dolfinx.mesh.CellType.tetrahedron, dolfinx.mesh.CellType.hexahedron])
def test_replacement_operator(cell_type, N, q_deg):
    ...
    N = FEMExternalOperator(x, theta, function_space=Q, external_function=lambda derivatives: u_NN_external(mesh.geometry.dim, derivatives), name="exop")
    ...
    def compute_J(N, phih):
        return ufl.inner(ufl.grad(phih), ufl.grad(phih)) * dx + alpha*ufl.inner(N, N)*dx
