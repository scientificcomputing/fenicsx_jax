...
import basix
import dolfinx
from dolfinx_external_operator import replace_external_operators, evaluate_operands, evaluate_external_operators
import dolfinx_external_operator.fem
import scifem
import numpy as np

__all__ = ["compile_external_operator_form", "pack_external_operator_data", "ExternalOperatorLinearProblem", "create_real_functionspace"]

def compile_external_operator_form(form: dolfinx.fem.Form, jit_options:dict|None=None, form_compiler_options:dict|None=None,
                                   entity_maps: list[dolfinx.mesh.EntityMap]|None=None) -> dolfinx.fem.Form:
    form_replaced, ex_ops = replace_external_operators(form)
    compiled_form = dolfinx.fem.form(form_replaced, jit_options=jit_options, form_compiler_options=form_compiler_options, entity_maps=entity_maps)
...
class LinearProblem(dolfinx.fem.petsc.LinearProblem):
    def __init__(self, a, L, u=None, bcs=None, petsc_options=None, petsc_options_prefix=None, entity_maps=None):
        a_replaced, a_ex_ops = replace_external_operators(a)
        L_replaced, L_ex_ops = replace_external_operators(L)
        super().__init__(a_replaced, L_replaced, u=u, bcs=bcs, petsc_options=petsc_options, petsc_options_prefix=petsc_options_prefix, entity_maps=entity_maps)
...
def create_real_functionspace(
    ...
    if (dtype := mesh.geometry.x.dtype) == np.float64:
        cppV = scifem._scifem.create_real_functionspace_float64(mesh._cpp_object, value_shape)
    elif dtype == np.float32:
        cppV = scifem._scifem.create_real_functionspace_float32(mesh._cpp_object, value_shape)
    else:
